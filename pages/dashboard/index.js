...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const grouped = suppsData.reduce((acc, supp) => {
  ...
  const TimeBlock = ({ title, supplements, icon: Icon, bgColor, borderColor }) => (
    <div className={`bg-white rounded-xl shadow-sm p-6 border-l-4 ${borderColor}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {supplement.currentDosage.amount} {supplement.currentDosage.unit}
                </p>
              </div>
              <button
        ...
        {supplements.length === 0 && (
          <p className="text-gray-500 col-span-2">No supplements scheduled for this time</p>
        )}
      </div>
    </div>
  );
